...
import PetsModel from "./pet-type.schema";
...
export const getAllPets = async (payload) => {
  ...
  const result = await PetsModel.paginate(query, options);
...
export const createPetType = async (payload) => {
  ...
  const existed = await PetsModel.findOne({
    $or: [{ name: name }],
  });
  ...
  const newPetType = await PetsModel.create({
    ...payload,
  });
...
export const deleteManyPets = async (ids) => {
  ...
  const existingDocs = await PetsModel.find(
  ...
  const deleteResult = await PetsModel.deleteMany({
    _id: { $in: validObjectIds },
  });
